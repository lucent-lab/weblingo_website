...
import styles from "./variant-35.module.css";

export function LandingVariant35({
  locale,
  messages,
  t,
  content,
  ...
  return (
    <div className={cn(styles.root, "min-h-screen bg-background")}>
      <div aria-hidden className={styles.gridBg} />
        ...
        <div className="relative mx-auto max-w-6xl">
          <div className="grid items-center gap-12 lg:grid-cols-[1.1fr_0.9fr]">
            ...
            >
              <div
                className={cn(
                  "pointer-events-none absolute -inset-8 rounded-3xl bg-primary/10 blur-2xl",
                  styles.formPulse,
                )}
              />
              <div
      ...
      <section className="border-y border-border bg-secondary/50 px-4 py-16 sm:px-6 sm:py-24 lg:px-8">
        <div className="mx-auto max-w-6xl">
          <div className="grid gap-12 lg:grid-cols-[0.95fr_1.05fr] lg:items-center">
            <div>
              <h2 className={cn("mb-4 text-3xl font-bold sm:text-4xl", styles.gradientText)}>
      ...
      {/* How it works - card style with slide hover */}
      <section
        id="how-it-works"
          ...
          <div className="space-y-6">
            {content.how.items.map((item, index) => (
              <div
                key={item.titleKey}
                className={cn(
                  "flex gap-6 rounded-2xl border border-border bg-card p-5",
                  styles.stepCard,
                )}
              >
                <div
                  className={cn(
                    "flex h-12 w-12 flex-shrink-0 items-center justify-center rounded-lg bg-primary font-bold text-primary-foreground",
                    styles.stepNum,
                  )}
                >
                  {index + 1}
          ...
          <div className="grid gap-6 md:grid-cols-2">
            {LANDING_FAQ_ITEMS.map((item) => (
              <div
                ...
                className={cn("rounded-2xl border border-border bg-card p-6", styles.faqAccordion)}
              >
