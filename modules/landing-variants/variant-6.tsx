...
import styles from "./variant-6.module.css";
...
export function LandingVariant6({
  locale,
  messages,
  t,
  content,
  ...
  return (
    <div className={cn(styles.root, unbounded.variable, hanken.variable, "min-h-screen")}>
      ...
      <header className="sticky top-0 z-40 border-b border-border bg-background/80 backdrop-blur">
        <div className="mx-auto flex max-w-6xl items-center justify-between gap-4 px-4 py-4 sm:px-6">
          <Link href={`/${locale}`} className="group inline-flex items-center gap-3">
            <span className="grid h-10 w-10 place-items-center rounded-xl border border-border bg-card shadow-sm transition group-hover:-rotate-2">
      ...
      <section id="try" className="relative overflow-hidden px-4 pb-16 pt-12 sm:px-6 sm:pb-24 sm:pt-16">
        <div aria-hidden className={styles.shapes} />

        <div className="mx-auto grid max-w-6xl items-start gap-10 lg:grid-cols-[1.12fr_0.88fr] lg:items-start lg:gap-12">
          <div className={cn(styles.enter)} style={{ animationDelay: "50ms" }}>
            <div className="inline-flex flex-wrap items-center gap-3">
              <span
                className={cn(
                  styles.badge,
                  "relative rounded-full border border-border bg-card px-4 py-2 text-[11px] font-semibold uppercase tracking-[0.28em] text-foreground shadow-sm",
                )}
              >
            ...
            >
              <span className="relative inline-block">
                <span className="absolute -inset-x-2 -inset-y-1 -z-10 rotate-[-1deg] rounded-2xl bg-accent/15" />
          ...
          <aside className={cn(styles.enter)} style={{ animationDelay: "120ms" }}>
            <div className={cn(styles.slice, "rounded-[1.5rem] p-1 shadow-xl")}>
              <div className="rounded-[1.35rem] bg-card/90 p-6 backdrop-blur">
            ...
            <div className="mt-6 grid grid-cols-3 gap-3">
              {content.stats.map((stat, idx) => (
                <div
                  ...
                  className={cn(
                    styles.enter,
                    "rounded-2xl border border-border bg-card/70 px-4 py-4 shadow-sm",
                  )}
      ...
      <section className="border-y border-border bg-secondary/60 px-4 py-16 sm:px-6 sm:py-24">
        <div className="mx-auto grid max-w-6xl gap-12 lg:grid-cols-[0.95fr_1.05fr] lg:items-start">
          <div>
            ...
            <div className="mt-8 rounded-3xl border border-primary/20 bg-primary/10 p-6 shadow-sm">
          ...
          <div className="grid gap-6">
            {content.pain.items.map((item) => (
              <div
                key={item.titleKey}
                className="group rounded-3xl border border-border bg-card p-6 shadow-sm transition hover:-translate-y-0.5 hover:border-foreground/20"
              >
                <div className="flex items-start gap-4">
                  <div className="mt-0.5 grid h-12 w-12 place-items-center rounded-2xl bg-accent/10 text-accent shadow-sm transition group-hover:-rotate-2">
        ...
        <div className="mx-auto max-w-6xl">
          ...
          <div className="mt-10 grid gap-6 md:grid-cols-3">
            {content.useCases.items.map((item) => (
              <div
                key={item.titleKey}
                className="relative overflow-hidden rounded-3xl border border-border bg-card p-6 shadow-sm transition hover:-translate-y-0.5 hover:border-primary/25"
              >
                <div className="pointer-events-none absolute -right-10 -top-10 h-32 w-32 rounded-full bg-primary/10 blur-2xl" />
                <div className="mb-4 grid h-12 w-12 place-items-center rounded-2xl bg-primary/10 text-primary shadow-sm">
                  <item.icon className="h-5 w-5" />
                </div>
                <h3 className="text-lg font-semibold text-foreground">{t(item.titleKey)}</h3>
        ...
        <div className="mx-auto max-w-5xl">
          ...
          <div className="mx-auto mt-12 grid max-w-3xl gap-6">
            {content.how.items.map((step, index) => (
              <div key={step.titleKey} className="rounded-3xl border border-border bg-card p-6 shadow-sm">
                <div className="flex gap-5">
                  <div className="grid h-12 w-12 flex-shrink-0 place-items-center rounded-2xl bg-accent text-accent-foreground shadow-sm">
            ...
            <div className="grid gap-4 md:grid-cols-2">
              {LANDING_FAQ_ITEMS.map((item) => (
                <details
                  ...
                  className="group rounded-3xl border border-border bg-card p-6 shadow-sm open:border-primary/30"
                >
