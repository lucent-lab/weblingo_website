...
# Upstream ingestion host. Browser requests are proxied through /api/analytics/posthog.
...
PostHog traffic is proxied through the first-party `/api/analytics/posthog` route. Keep `NEXT_PUBLIC_POSTHOG_HOST` pointed at the upstream PostHog ingestion host (for example `https://eu.i.posthog.com`), not the browser-facing proxy path.
