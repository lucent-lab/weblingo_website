# WebLingo — MVP (Cloudflare + Supabase) 🗺️

A production‑ready **MVP** for static‑page website translation built around a **deterministic edit manifest**, **Cloudflare Workers/Queues/R2/KV**, and **Supabase/Postgres** as the source of truth. The design is **TDD‑first**, keeps **token cost** low, and is **reverse‑proxy ready** for dynamic pages later.

> **Hot path (serving):** KV → R2 (DB‑free).  
> **Pipeline:** Crawl/Render → Extract → Translate (precedence) → **Build manifest** → Apply → Publish → Serve.
...
# 7) Run serve worker locally (DB‑free hot path)
corepack pnpm dev:serve  # wrapper for wrangler dev in workers/serve-worker
```

Wrangler dev does **not** read the root `.env`. Before running `dev:serve`, `dev:webhooks`, or any other worker locally, create a `.dev.vars` file inside that worker’s directory and copy the Supabase values generated by `supabase start` (see `.env`). Example:
...
Repeat for `workers/webhooks-worker/.dev.vars` so the onboarding API can reach Supabase. Without these files Wrangler will throw errors such as “Supabase URL is required.”
...
- Milestone **M11 — Context-Aware Manifest (CAM v2)** introduces Site/Page/Group context capture. Run the migrations in `infra/supabase/migrations` that add `sites.context_profile`, `page_versions.meta_context`, and `segment_groups`, then confirm `workers/extract-worker` stores those rows before enabling CAM tests.
...
- `build` — build every package/worker with tsup.
- `test` — Vitest unit suite with coverage (also part of `check`).
- `lint` / `type` / `format` — ESLint, `tsc --noEmit`, and Prettier checks.
- `check` — runs `lint`, `type`, and `test` in sequence (mirrors CI).
- `db:migrate` — applies SQL in `infra/supabase/migrations` via `scripts/db-migrate.ts`.
- `seed:tm` — upserts sample translation memory rows into the local Supabase instance (`scripts/seed-segment-targets.ts`).
- `demo:translator` — runs segmenter + translator CLI (with Supabase persistence) via `scripts/run-translator-demo.ts` and verifies there is at least one TM hit.
- `e2e:mini` — placeholder Playwright spec under `tests/e2e`.
- `e2e:supabase` — spins up an isolated Supabase snapshot and runs the supabase-backed pipeline specs (`tests/e2e/pipeline.supabase.test.ts`). See `docs/E2E_SCENARIOS.md` for the scenario matrix.
- `segmenter:cli` — run the segmenter against an HTML snapshot (`corepack pnpm segmenter:cli fixtures/html/about.html`).
- Worker helpers:
  - `dev:<worker>` (`crawl`, `extract`, `translate`, `render`, `publish`, `serve`, `webhooks`) → `wrangler dev` inside that worker.
  - `deploy:<worker>` → `wrangler deploy` for that worker (provide real bindings/secrets first).
  - `wrangler` — pass-through `corepack pnpm exec wrangler` for ad-hoc commands.
- `blocks:list` — list content blocks + members for a site (`SUPABASE_URL` + `SUPABASE_SECRET_KEY` required): `corepack pnpm blocks:list <siteId>`.
...
Need to update locales later? `PATCH /api/sites/:id` with a new `targetLangs` array and/or `subdomainPattern`. A `POST /api/sites/:id/crawl` call replays the crawl → extract pipeline via the crawl queue, so onboarding can run entirely from the API.
...
> ⚠️ Reminder: create `.dev.vars` files in both `workers/serve-worker` and `workers/webhooks-worker` with `SUPABASE_URL`, `SUPABASE_PUBLISHABLE_KEY`, and `SUPABASE_SECRET_KEY` (copy from `.env`). Wrangler doesn’t load root env files, so missing entries surface as `Supabase URL is required` errors when you hit `wrangler dev`.
...
- **Production deploy**: Set secrets per worker in your Cloudflare account with `wrangler secret put` from inside each worker directory (or use `--name <worker>`). Example for translate-worker:
  ...
  wrangler secret put OPENAI_API_KEY
  ```
  Repeat for other workers’ secrets (e.g., `WEBHOOK_SECRET`, `TRY_NOW_TOKEN`, `DIGEST_*`, `CRAWL_TRIGGER_TOKEN`). Use `--env dev` if you maintain separate dev/stage environments.
...
- **Segmenter**: HTML → blocks → placeholders → ICU segmentation → `seg_id`, `context_hash`, `dom_locator`.
- **Translator**: precedence **overrides → glossary → TM → MT**; strict placeholder validation; batch MT.
- **Manifest**: schema (selector + anchors + `expected_source` + checksum). Builder & **idempotent** applier.
- **Workers/Queues**: `crawl` → `extract` → `translate` → `render` → `publish`. All idempotent (KV).
- **Serve worker**: KV → R2 mapping; cache; rollback via `prev.html`.
- **SEO**: canonical + hreflang with localized slugs/meta tags; per‑locale sitemaps; link rewriting to locale hosts.
...
If Wrangler can’t reach Supabase, double-check the `.dev.vars` files in each worker directory contain the `SUPABASE_*` values from `.env`.
...
Open **docs/TASKS_CHECKLIST.md**, work milestone by milestone. Do not proceed until **DoD** is complete. Run the per‑milestone commands from the guide after each step.  
Post‑MVP milestones (M7+) reference `docs/POST_MVP_ROADMAP.md`; read the cited sections there before starting any roadmap tasks (e.g., the self‑service onboarding API in §§1.1–1.2, 8.3 will live inside the existing Cloudflare workers so the Next.js/Stripe frontend can call into it).

---

## License

Proprietary — internal project bootstrap.
