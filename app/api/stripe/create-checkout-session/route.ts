...
export async function POST(request: NextRequest) {
  ...
  try {
    const session = await createCheckoutSession({
      ...
      cadence,
      email,
      successUrl: `${env.NEXT_PUBLIC_APP_URL}/${normalizedLocale}/checkout/success?session_id={CHECKOUT_SESSION_ID}`,
      cancelUrl: `${env.NEXT_PUBLIC_APP_URL}/${normalizedLocale}/checkout/cancel`,
    });
