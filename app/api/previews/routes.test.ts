...
describe("/api/previews proxy routes", () => {
  ...
  test("POST /api/previews returns 503 and logs cause details when rate limit backend fails", async () => {
    vi.resetModules();
    const { POST } = await import("./route");
  ...
  test("GET /api/previews/:id returns 503 when the rate limit backend fails", async () => {
    vi.resetModules();
    const { GET } = await import("./[id]/route");
  ...
  test("GET /api/previews/:id/stream returns 503 when the rate limit backend fails", async () => {
    vi.resetModules();
    const { GET } = await import("./[id]/stream/route");
